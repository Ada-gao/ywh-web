<template>
  <div class="page-account">
    <el-card>
      <el-col :span="12">
        <div class="logo-one">
          <i class="iconfont icon-yue-copy"/>
        </div>
        <div class="info">
          <div class="title">
            <span>账户余额</span>
          ...
          <div class="count-one">
            {{account.balance?(account.balance*0.01).toFixed(2):'0.00'}}<span>元</span>
          </div>
        </div>
        <div class="tip" v-show="account.balance < account.balanceThreshold">
      ...
      <el-col :span="12">
        ...
        <div class="info">
          ...
          <div class="count-two">
            {{account.totalConsumption?(account.totalConsumption * 0.01).toFixed(2):'0.00'}}<span>元</span></div>
        </div>
      </el-col>
    </el-card>
    <div class="com_page">
      <div class="item">
        <div class="com_head">
          <span class="com_title">账户信息<span>（变更账号信息，请联系对接商务经理，谢谢）</span></span>
        </div>
        <el-row>
          ...
          <el-col :span="8"><span class="detail-label">Account ID:</span><span class="detail-value" style="width: 70%">{{account.accountId}}</span></el-col>
          <el-col :span="8"><span class="detail-label">账户到期时间:</span><span class="detail-value">{{account.expireDate?account.expireDate:'长期有效'}}</span></el-col>
      ...
      <div class="item">
        <div class="com_head">
          <span class="com_title">公司信息<span>（变更公司信息，请联系对接商务经理，谢谢）</span></span>
        </div>
        <el-row>
          <el-col :span="8"><span class="detail-label">公司ID:</span><span class="detail-value">{{company.companyCode}}</span></el-col>
          <el-col :span="8"><span class="detail-label">公司名称:</span><span class="detail-value">{{company.companyName}}</span></el-col>
          <el-col :span="8"><span class="detail-label">公司简称:</span><span class="detail-value">{{company.shortName?company.shortName:'-'}}</span></el-col>
          <el-col :span="8"><span class="detail-label">所属行业:</span><span class="detail-value">{{company.industryType}}</span></el-col>
          <el-col :span="8"><span class="detail-label">公司规模:</span><span class="detail-value">{{company.orgSize}}</span></el-col>
          <el-col :span="8"><span class="detail-label">所在地:</span><span class="detail-value">{{company.companyProvince}}</span></el-col>
          <el-col :span="8"><span class="detail-label">公司地址:</span><span class="detail-value">{{company.companyAddress}}</span></el-col>
          <el-col :span="8"><span class="detail-label">联系人:</span><span class="detail-value">{{company.contact}}</span></el-col>
          <el-col :span="8"><span class="detail-label">联系人电话:</span><span class="detail-value">{{company.contactMobile}}</span></el-col>
          <el-col :span="8"><span class="detail-label">职务:</span><span class="detail-value">{{company.occupation}}</span></el-col>
          <el-col :span="8"><span class="detail-label">企业微信:</span><span class="detail-value">{{company.wechatNo?company.wechatNo:'-'}}</span></el-col>
          <el-col :span="8"><span class="detail-label">公司logo:</span><span class="detail-value"><img :src.sync="company.logo" alt="" style="width: 120px; height: 80px;"></span>
          </el-col>
        </el-row>
      </div>
      <div class="item">
        <div class="com_head">
          <span class="com_title">消费记录</span>
      ...
      <div class="item">
        <div class="com_head">
          <span class="com_title">充值记录</span>
        ...
        <div v-show="!listLoading3">
          <div class="total">累计充值金额：{{(rechargeMoney * 0.01).toFixed(2)}}元</div>
        ...
        <el-table :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row>
          ...
          <el-table-column label="状态">
            <template slot-scope="scope">
              <div class="switch">
                <el-switch class="sw" v-model="scope.row.enabled" active-color="#0299CC" inactive-color="#C0CCDA" @change="switchMode(scope.row.id, scope.row.enabled)"/>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <a @click="showUpdateInfoDialog(scope.row)">修改信息</a> | <a @click="showUpdatePwdDialog(scope.row)">修改密码</a>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="item">
        <div class="com_head">
          <span class="com_title">登录情况</span>
        </div>
        <el-table :data="list4" v-loading="listLoading4" element-loading-text="给我一点时间" border fit highlight-current-row>
          <el-table-column label="登录账号">
      ...
      <el-dialog title="修改密码" :visible.sync="updatePwdDialog">
        <el-form :model="adminForm" :rules="adminRules" ref="pwdForm" label-width="80px" style="margin-right: 20px;">
